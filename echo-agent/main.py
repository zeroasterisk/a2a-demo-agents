"""
Echo Agent — A2A demo agent that echoes messages back to sender.

Registers with relay on startup, serves A2A tasks/send endpoint,
and replies with "Echo: <original text>" to the sender agent.
"""
...
AGENT_DESCRIPTION = "Echoes messages back to sender"
...
async def poll_and_reply_loop():
    """Poll relay mailbox every 2s and echo any queued messages back."""
                        ...
                        for msg in params.get("messages", []):
                            for part in msg.get("parts", []):
                                if part.get("type") == "text":
                                    text = part.get("text", "")
                        if sender and sender != AGENT_ID and text:
                            reply = {
                                "jsonrpc": "2.0", "method": "tasks/send", "id": "poll-reply",
                                "params": {
                                    "id": str(uuid.uuid4()),
                                    "metadata": {"relay_target_agent_id": sender, "sender_agent_id": AGENT_ID},
                                    "messages": [{"role": "user", "parts": [{"type": "text", "text": f"Echo: {text}"}]}],
                                },
                            }
                            await client.post(f"{RELAY_URL}/a2a", json=reply,
                                headers={"Authorization": f"Bearer {app.state.bearer_key}"})
                            logger.info(f"Echoed '{text[:40]}' back to {sender}")
                        # ACK the message
...
async def startup():
    logger.info(f"Echo Agent starting on port {AGENT_PORT}, relay={RELAY_URL}")
        ...
        "skills": [
            {
                "id": "echo",
                "name": "Echo",
                "description": "Echoes any message back to sender",
                "tags": ["echo", "demo"],
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent_id": AGENT_ID, "registered": app.state.bearer_key is not None}
...
async def handle_a2a(request: Request):
    ...
    try:
        ...
        logger.info(f"Message from '{sender}': {text!r}")
